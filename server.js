...
const otpStore = {};
const users = {}; 
const chatHistory = {}; // Naya: Chat history save karne ke liye

// --- Nodemailer Transport ---
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: { user: 'sheikhtaj466@gmail.com', pass: 'eluplxqapjdtwpsh' }
});

// --- API Endpoints ---
app.post('/send-otp', (req, res) => { /* Ismein koi badlaav nahi */ 
...
// Nayi API: Do users ke beech ki chat history bhejegi
...
io.on('connection', (socket) => {
    socket.on('register', (email) => {
        ...
        socket.email = email;
        io.emit('user status changed', { email, online: true, lastMessage: users[email].lastMessage, timestamp: users[email].timestamp });
    });
    
    socket.on('private message', ({ to, message }) => {
        const recipient = users[to];
        const sender = users[socket.email];
        const roomKey = [to, socket.email].sort().join('-');
        ...
        chatHistory[roomKey].push(messageData);
        ...
        // Update contact lists for everyone
        io.emit('user status changed', { email: to, online: recipient.online, lastMessage: message, timestamp });
        io.emit('user status changed', { email: socket.email, online: sender.online, lastMessage: message, timestamp });
    });

    socket.on('disconnect', () => {
        if (socket.email && users[socket.email]) {
            users[socket.email].online = false;
            io.emit('user status changed', { email: socket.email, online: false, lastMessage: users[socket.email].lastMessage, timestamp: users[socket.email].timestamp });
        }
    });
});
